...
class Explorer
  ...
  def check_surroundings(path)
    ...
    DIRECTIONS.each do |dir|
      spaces      << current.next_(dir) if can_continue?(path, dir)
      end_of_maze =  current.next_(dir) if found_end?(path, dir)
    end
  ...
  def found_end?(path, dir)
    @maze.end?(path.last.next_(dir))
  end
